...
class TensorGraph:
    ...
    def build(self, progress):
        ...
        for k, (v, trainable) in sub(self.base_arrays_init.items()):
            ...
            var = RefVariable(
                ...
                trainable=trainable,
                name="%s/%s" % (
                    "trainable_vars" if trainable else "local_vars", name),
            )
    ...
    def build_optimizer_func(self, optimizer, loss, direct_grads=None):
        ...
        key = (optimizer,
               frozenset(loss.items()),
               frozenset(direct_grads))
    ...
    def build_outputs(self, outputs):
        ...
        new_vars.extend(
            set(self.graph.get_collection("gradient_vars")
