...
def test_build_outputs_variables(Simulator):
    ...
    with Simulator(net, minibatch_size=5) as sim:
        ...
        _, init_op = sim.tensor_graph.build_outputs(
            {(p_a, p_b): func, p_b: func2})
    ...
    with Simulator(net) as sim:
        ...
        class Loss:
            def pre_build(self, *_):
                self.var = RefVariable(
                    tf.constant(2.0, dtype=sim.tensor_graph.dtype), name="two",
                    trainable=False)
                return self.var
...
def test_get_tensor(Simulator):
    ...
    with Simulator(net) as sim:
        ...
        assert np.allclose(sim.sess.run(tensor, feed_dict=sim._internal_state),
                           np.arange(10)[:, None])
