...
def test_signal_dict_scatter(sess):
    ...
    y = signals.get_tensor_signal([0, 1, 2, 3], var_key, tf.float32, (4,),
                                  True)
...
def test_tf_indices_nd(sess, ndims):
    ...
    shape = (3, 4, 5)[:ndims]
    x = tf.ones(shape) * tf.reshape(tf.range(0, 3, dtype=tf.float32),
                                    (-1,) + (1,) * (ndims - 1))
    assert x.shape == shape
    sig = signals.get_tensor_signal(
        [0, 2], None, np.float32, shape, False)
