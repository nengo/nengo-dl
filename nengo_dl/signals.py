...
class SignalDict(Mapping):
    ...
    def make_internal(self, name, shape, minibatched=True):
        ...
        ph = tf_compat.placeholder(
            sig.dtype, sig.full_shape,
            name="%s/%s" % (tf_compat.get_default_graph().get_name_scope(),
                            name),
        )

        self.base_tensors[sig.key] = (
            ph, np.zeros(sig.full_shape, dtype=sig.dtype.as_numpy_dtype()))
    ...
    def constant(self, value, dtype=None, cutoff=1 << 25):
        ...
        if value.nbytes > cutoff:
            ph = tf_compat.placeholder(
                dtype, value.shape,
                name="constant_phs/%d" % len(self.constant_phs))
